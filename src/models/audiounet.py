...
from keras.layers import merge, concatenate, add, Dropout
...
#from keras.initializations import normal, orthogonal # in Keras 1
    ...
    def create_model(self, n_dim, r):
        ...
        for l, nf, fs in zip(range(L), n_filters, n_filtersizes):
            with tf.name_scope('downsc_conv%d' % l):
                # in Keras 2
                x = Conv1D(padding='same', kernel_initializer='Orthogonal', filters=nf, kernel_size=fs, activation=None)(x)
        ...
        with tf.name_scope('bottleneck_conv'):
            # in Keras 2
            x = Conv1D(padding='same', kernel_initializer='Orthogonal', 
                       filters=n_filters[-1], kernel_size=n_filtersizes[-1], activation=None)(x)
        ...
        for l, nf, fs, l_in in zip(reversed(range(L)), reversed(n_filters), reversed(n_filtersizes), reversed(downsampling_l)):
            with tf.name_scope('upsc_conv%d' % l):
                ...
                # in Keras 2
                x = Conv1D(padding='same', kernel_initializer='Orthogonal', 
                           filters=2*nf, kernel_size=fs, activation=None)(x)
        ...
        with tf.name_scope('lastconv'):
            # in Keras 2
            x = Conv1D(padding='same', kernel_initializer='he_normal', 
                       filters=2, kernel_size=9, activation=None)(x)
