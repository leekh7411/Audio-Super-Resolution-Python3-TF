...
def upsample_wav(wav, args, model):
    ...
    # upscale the low-res version using trained model
    X_hr = x_hr.reshape((1,len(x_hr),1))
    X_lr = x_lr.reshape((1,len(x_lr),1))
    print(X_hr.shape)
    print(X_lr.shape)
    P = model.predict(X_lr,X_hr)
    x_pr = P.flatten()
    ...
    librosa.output.write_wav(outname + '.lr.wav', x_lr, int(fs/args['r']))  
    ...
    n_fft = int(2048/args['r'])
    S = get_spectrum(x_lr, n_fft=n_fft)
